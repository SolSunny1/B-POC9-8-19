trigger TaskTrigger_BKP on Task_BKP (after insert,after update) {
  list<ObjectFieldMapping__c> lstObjectFieldMappingInfo=ActivityHelper.getObjectFieldMappingInfo('LastRelationshipType-Condition','Task');
    ...
    List<Task> taskContactsLst = [SELECT Id (SELECT RelationId, TaskId FROM TaskRelations) FROM Task WHERE id in:setIds];
    System.debug('Print Task Contacts: ' + taskContacts);
    for(Task tsk: taskContactsLst){
            for(TaskRelation tRel: tsk.TaskRelations){
                System.debug('Print Task Contacts: ' + tRel); 
                String relationKeyPrefix = String.valueOf( tRel.relationId ).left( 3 );
                if (tRel.size > 0 && contactKeyPrefix == relationKeyPrefix ){                    // checking if size is greater than 0 
                    mapContact.add(tRel.id);             // then add in contactId list
                }  
            }
        }
