    ...
    if(Trigger.isUpdate){
        for(contact new_c: Trigger.new){
            if(new_c.Do_Not_Auto_Update_Owner__c == false){  //SS: 6-27-19 moved this if statement before below old_c line. 
                ...
                }else if(new_c.MailingPostalCode != old_c.MailingPostalCode && new_c.MailingPostalCode != null && new_c.MailingPostalCode != '' ||
                   new_c.AutoUpdateOwner__c != old_c.AutoUpdateOwner__c && new_c.AutoUpdateOwner__c == true){
        ...
        for(contact c: Trigger.new){
            if(c.Do_Not_Auto_Update_Owner__c == false){   //SS: 6-27-19 appended this if statement before collection statement below to bypass contacts with do not auto update flag. 
                ...
                for(Assignments__c objAssignment: lstAssignments){
                    if(objAssignment.Use_Zip_Code__c == true ){
                        if (c.MailingPostalCode == objAssignment.Zip_Code__c){ // && 
                   ...
                   }else if (objAssignment.Use_Zip_Code__c == false){ 
                            if (c.MailingState == objAssignment.State__c && 
                                (objAssignment.Zip_Code__c == '' || objAssignment.Zip_Code__c == null)){
                    ...
                    /*
                    else if(objAssignment.Use_Zip_Code__c == true && 
                            c.MailingState == objAssignment.State__c && 
                       		(objAssignment.Zip_Code__c == '' || objAssignment.Zip_Code__c == null)){ // Condition 3 of functinality i.e If Use_ZipCode is true, match first 5 digiits  of MailingPostalCode on Contact to PostalCode__c on Assignment and set Contact Owner to Owner__c,...
                            //if(){
                                 tempContact.OwnerID = objAssignment.Owner__c;
                                 isOwnerAssigned = true;
