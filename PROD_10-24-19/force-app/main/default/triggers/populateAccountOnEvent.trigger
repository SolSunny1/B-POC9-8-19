//Trigger to autopopulate Custom account field on Event
trigger populateAccountOnEvent on Event (before insert,before update, before delete){
      ...
      if(trigger.isUpdate){
            ...
            if(!LatestActivity){
                ...
                if(setProfiles != null && setProfiles.size() > 0 && profileName!= null && setProfiles.contains(profileName)){
                    ...
                    //Getting the older activities of the object
                    if(id != null && (id .startsWith('001') || id .startsWith('a0p'))){
                        lstTasks = [SELECT Id,ActivityDate,Latest_Activity__c  FROM Task WHERE Account__c = : id AND IDI_GazleyActivity__c = TRUE AND Latest_Activity__c = TRUE ];
                        lstEvents = [SELECT Id,ActivityDate,Latest_Activity__c  FROM Event WHERE Account__c = : id AND IDI_GazleyActivity__c = TRUE AND Latest_Activity__c = TRUE ];
                        System.debug('++++++lstTasks ++++++ :'+lstTasks );
                    }
    ...
    if(trigger.isDelete){
        ...
        if(LatestActivity){
            ...
            if(setProfiles != null && setProfiles.size() > 0 && profileName!= null && setProfiles.contains(profileName)){
                    ...
                    //Getting the older activities of the object
                    if(id != null && (id .startsWith('001') || id .startsWith('a0p'))){
                        lstTasks = [SELECT Id,LastModifiedDate ,Latest_Activity__c,Account__c   FROM Task WHERE Account__c = : id AND IDI_GazleyActivity__c = TRUE AND Latest_Activity__c = FALSE ORDER BY LastModifiedDate DESC LIMIT 1];
                    ...
                    //System.debug('++++++lstTasks LastModifiedDate  ++++++ :'+lstTasks[0].LastModifiedDate  );
                    //System.debug('++++++lstEvents LastModifiedDate  ++++++ :'+lstEvents[0].LastModifiedDate   );
