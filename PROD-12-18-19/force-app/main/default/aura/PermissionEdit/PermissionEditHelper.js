({
                ...
                jQuery(document).ready(function() {
                    ...
                    $window.scroll(function() {
                        ...
                        if(!tableHeaderIsStuck && windowScrollTop >= tableHeaderRelPos) {
                            ...
                            $tableHeader.css('top',tableHeaderTop-multiSelectPanelHeight-14); //2 px for border top/bottom, 12 px for top padding.
    ...
    initTable: function(cmp, returnValue){
        ...
        if ($A.util.isEmpty(cmp.get("v.existingPermissions"))) {
            var noRecordWarning = $A.get("e.force:showToast");
    ...
    initRelatedSelect: function(cmp){
        ...
        const relatedRecordDefaults = cmp.get("v.relatedRecordDefaults");
        const widgetWrapper = [];
    ...
    setPermissionFieldValues : function(pms, fieldName, fieldValue) {
        if (!(pms.disableStatus && fieldName === "Status__c") && !(pms.disableAlert && fieldName === "Alert_Status__c")) {
    ...
    setRowLevelOptions : function(row, optionKey, optionValue, optionIndex) {
        var self = this;
        if (optionIndex < 0) { //Row Option Control
            self.setPermissionFieldValues(row.pmsRowCtrl, optionKey, optionValue);
            row.pmsList.forEach(function (pms) {
                self.setPermissionFieldValues(pms, optionKey, optionValue);
            });
        } else { //Column Option Control
            self.setPermissionFieldValues(row.pmsList[optionIndex], optionKey, optionValue);
        }
    ...
    setKeyLevelOptions : function(keyObj, optionKey, optionValue, optionIndex) {
        var self = this;
        if (optionIndex < 0) { //Row Option Control
            self.setPermissionFieldValues(keyObj.pmsRowCtrl, optionKey, optionValue);
            keyObj.pmsList.forEach(function(pms) {
                self.setPermissionFieldValues(pms, optionKey, optionValue);
            });
            keyObj.rows.forEach(function(row) {
                self.setRowLevelOptions(row, optionKey, optionValue, optionIndex);
            });
        } else { //Column Option Control
            self.setPermissionFieldValues(keyObj.pmsList[optionIndex], optionKey, optionValue);
            keyObj.rows.forEach(function(row) {
                self.setRowLevelOptions(row, optionKey, optionValue, optionIndex);
            });
        }
    ...
    setPermissionControls : function(cmp, keyObjects) {
        ...
        masterPmsList.forEach(function(pms) {
            //pms.permission.Status__c = initialOption;
            //pms.permission.Alert_Status__c = initialOption;
            pms.disableStatus = true;
            pms.disableAlert = true;

            pms.countActiveStatus = 0;
            pms.countActiveAlert = 0;
            pms.totalStatus = 0;
            pms.totalAlert = 0;
        });
        keyObjects.forEach(function(keyObj) {
            //initialOption = keyObj.rows.length > 0 ?  "Active" : "Inactive";
            //keyObj.pmsRowCtrl.permission.Status__c = initialOption;
            //keyObj.pmsRowCtrl.permission.Alert_Status__c = initialOption;
            ...
            keyObj.pmsList.forEach(function(pms) {
                ...
                pms.countActiveStatus = 0;
                pms.countActiveAlert = 0;
                pms.totalStatus = keyObj.rows.length;
                pms.totalAlert = keyObj.rows.length;
            });
            ...
            keyObj.rows.forEach(function(row) {
                ...
                //row.pmsRowCtrl.permission.Status__c = "Active";
                //row.pmsRowCtrl.permission.Alert_Status__c = "Active";
                row.pmsRowCtrl.disableStatus = true;
                row.pmsRowCtrl.disableAlert = true;
                for (var i = 0; i < row.pmsList.length; i++) {
                    if (row.pmsList[i].permission.Status__c === "Active") {
                        keyObj.pmsList[i].countActiveStatus += 1;
                        keyObj.pmsRowCtrl.countActiveStatus += 1;
                        row.pmsRowCtrl.countActiveStatus += 1;
                        masterPmsList[i].countActiveStatus += 1;
                    }

                    if (row.pmsList[i].permission.Alert_Status__c === "Active") {
                        keyObj.pmsList[i].countActiveAlert += 1;
                        keyObj.pmsRowCtrl.countActiveAlert += 1;
                        row.pmsRowCtrl.countActiveAlert += 1;
                        masterPmsList[i].countActiveAlert += 1;
                    }

                    /*if (row.pmsList[i].permission.Status__c === "Inactive") {
                        row.pmsRowCtrl.permission.Status__c = "Inactive";
                        keyObj.pmsList[i].permission.Status__c = "Inactive";
                        keyObj.pmsRowCtrl.permission.Status__c = "Inactive";
                        masterPmsList[i].permission.Status__c = "Inactive";
                    }
                    if (row.pmsList[i].permission.Alert_Status__c === "Inactive") {
                        row.pmsRowCtrl.permission.Alert_Status__c = "Inactive";
                        keyObj.pmsList[i].permission.Alert_Status__c = "Inactive";
                        keyObj.pmsRowCtrl.permission.Alert_Status__c = "Inactive";
                        masterPmsList[i].permission.Alert_Status__c = "Inactive";
                    }*/

                    if (!row.pmsList[i].disableStatus) {
                        row.pmsRowCtrl.disableStatus = false;
                        keyObj.pmsList[i].disableStatus = false;
                        keyObj.pmsRowCtrl.disableStatus = false;
                        masterPmsList[i].disableStatus = false;
                    }
                    if (!row.pmsList[i].disableAlert) {
                        row.pmsRowCtrl.disableAlert = false;
                        keyObj.pmsList[i].disableAlert = false;
                        keyObj.pmsRowCtrl.disableAlert = false;
                        masterPmsList[i].disableAlert = false;
                    }
                }
            ...
            for (var i = 0; i < keyObj.pmsList.length; i++) {
                self.setPermissionStatusButtonValue(keyObj.pmsList[i]);
                /*if (keyObj.pmsList[i].permission.Status__c === "Inactive") {
                    masterPmsList[i].permission.Status__c = "Inactive";
                }
                if (keyObj.pmsList[i].permission.Alert_Status__c === "Inactive") {
                    masterPmsList[i].permission.Alert_Status__c = "Inactive";
                }*/
                if (!keyObj.pmsList[i].disableStatus) {
                    masterPmsList[i].disableStatus = false;
                }
                if (!keyObj.pmsList[i].disableAlert) {
                    masterPmsList[i].disableAlert = false;
                }
            }
        });
        ...
        masterPmsList.forEach(function(pms) {
            self.setPermissionStatusButtonValue(pms);
        });
