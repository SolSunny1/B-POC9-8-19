({
        ...
        initAction.setCallback(this,function(response){
            ...
            setTimeout(function() {
                ...
                spinnerEvent.setParams({
                    "sender" : cmp.get("v.componentId"),
                    "receiver" : cmp.get("v.componentOrigin"),
                    "typeOfOperation" : "RELATED_MULTI_SELECT_DO_INIT_COMPLETION"
                });
                spinnerEvent.fire();
            });
        });
    ...
    initializeOptionList : function(cmp) {
        //For edit mode, selectedRecords will be a list of SObjectWrapper
        var self = this;
        const selectedRecords = cmp.get('v.selectedRecords');
        const displayFields = cmp.get("v.widget.AutoComplete_Target_Field__c").split(',');
        const options = [];
        const cmpId = cmp.get("v.componentId");
        for (let i = 0; i < selectedRecords.length; i++) {
            const wrapper = selectedRecords[i];
            const option = {
                label: '',
                id: wrapper.record.Id,
                selected: true,
                objectName: wrapper.objectName,
                name: wrapper.Name,
                record: wrapper.record,
                elementId: cmpId + "" + wrapper.record.Id
            };
            let optionLabels = [];
            for (let dIndex = 0; dIndex < displayFields.length; dIndex++) {
                optionLabels.push(self.getSObjectValue(wrapper.record, displayFields[dIndex].trim()));
        ...
        setTimeout(function() {
            ...
            const containerOffset = $("#" + cmpId + "-options-container").offset().top;
            let skipCounter = 0;
            for (let i = 0; i < options.length; i++) {
                const offsetKey = options[i].label.charAt(0).toLowerCase();
                if ($A.util.isEmpty(scrollOffsetMap[offsetKey])) {
                    scrollOffsetMap[offsetKey] = [];
                    skipCounter = 0;
                }
                if (skipCounter % 5 == 0)
                	scrollOffsetMap[offsetKey].push($("#"+options[i].elementId).offset().top - containerOffset);
            	skipCounter++;
            }
            console.log(scrollOffsetMap);
            cmp.set("v.scrollOffsetMap", scrollOffsetMap);
        });
    },
